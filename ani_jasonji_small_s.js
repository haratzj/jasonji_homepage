...
const params = {
    ...
    O: { o_arc: Math.PI, offsetX: 8, offsetY: 0, speed: 2, color: "black" },
    N: {
      n_index: 1,
      n_line: 1,
      n_line2: 2,
  ...
  animate = {
    ...
    rotateI: function (animatingParams, fullRotation = false) {
      ...
      if (fullRotation) animatingParams.rotation = 0.7;
      ctx.save();
...
function blank(withI = false, withDot = false, ctx) {
  ...
  ctx.beginPath();
  ctx.arc(0.5, 0, 1, -Math.PI / 8, (-Math.PI * 3) / 2, 1);
  ctx.arc(0.5, 2, 1, -Math.PI / 2 + 0.1, (Math.PI / 8) * 7);
  ctx.stroke();
  ctx.restore();
  //O
  ctx.save();
...
// ctx.translate(
//   scale + (jasonjiWidth - 20 * scale) / 2,
//   2 * scale + (canvas.height - 5 * scale) / 2
// );
// ctx.scale(scale, scale);
// ctx.lineWidth = 5 / ((scale * 2) / 5);
